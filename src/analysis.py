...
from preprocessing import load_data, inspect_and_clean, nlp_pipeline
...
def compute_global_occurrences(df: pd.DataFrame, top_n: int = 20) -> list:
    all_words = [word for tokens in df['tokens_cleaned'] for word in tokens]
...
def compute_occurrences_per_president(df: pd.DataFrame, top_n: int = 10) -> dict:
    ...
    for name, group in grouped:
        president_words = [word for tokens in group['tokens_cleaned'] for word in tokens]
...
def run_full_analysis_pipeline(file_path: str) -> pd.DataFrame:
    ...
    df_nlp = nlp_pipeline(df_cleaned)  # C'est ici qu'on applique la tokenisation/lemmatisation/POS/NER
